...
const Query = `
	query fetchImage($imageName: String!){
		fetchImage(imageName: $imageName, type: "labels") {
			data
		}
	}
`
...
class App extends Component {
  ...
  saveImageFromCanvas = async () => {
    this.setState({ processing: true })
    var image = new Image()
    image.id = "pic" + await getRandomInt()
    var canvas = document.getElementById("canvas");
    ...
    const fileName = await getRandomInt() + 'user_picture.png'
  ...
  onChange = async e => {
    ...
    this.setState({
      ...
      labelData: [],
      imageInfo: file,
      imageName: await getRandomInt() + file.name,
  ...
  saveFile = async(labels) => {
    ...
    if (this.state.imageName === '') return
		...
		try {
			await Storage.put(image.imageName, this.state.imageInfo);

			const result = await API.graphql(graphqlOperation(Query, image));
			const data= JSON.parse(result.data.fetchImage.data);

			console.log(`data`, JSON.parse(result.data.fetchImage.data));

			if(data) {
				this.setState({
					processing: false,
					rekognitionData: data.FaceDetails
				})
